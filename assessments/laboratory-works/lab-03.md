<!-- Источник: Методические указания к лабораторной работе 3_v2 с компенециями.docx -->
...
Выходной слой Dense устанавливается без функции активации, что означает, что на выходе будет просто линейное значение. Для преобразования этого значения в вероятность принадлежности к классу используется `from_logits=True` в функции потерь.
...
Предсказание интерпретируется: если значение выхода меньше 0.5 — это «КОТ», иначе — «СОБАКА». При этом, чем дальше значение предсказания уходит в отрицательную сторону, тем выше уверенность модели в том, что на изображении — кот. Аналогично, чем ближе значение к 1, тем выше уверенность в том, что на изображении собака.
